...
- **Detailed Calculation Tooltips:** Intricate step-by-step intermediate formula calculations (e.g. how diesel liters are derived physically, or how fNRB intersects with charcoal upstream assumptions) are exposed natively in the results formulas via hoverable `(i)` tooltips.
...
- **Anti-Double-Counting Guardrail:** Blocked with a UI warning if households are simultaneously active in Module E.
